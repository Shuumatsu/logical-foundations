...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite -> IHn. 
    assert (H: n + S n = S n + n). { apply plus_comm. }
...
Proof.
  ...
  assert (H: forall a b: nat, a * S b = a + a * b). { 
    intros. induction a.
      - reflexivity.
      - simpl. rewrite plus_swap. rewrite IHa. reflexivity.
  }
  induction n.
  - apply mult_0_r.
...
Theorem plus_swap' : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros. repeat rewrite plus_assoc.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite IHb. simpl. rewrite <- plus_n_O. rewrite plus_comm. reflexivity.
Qed.
(* FILL IN HERE *)
...
(* An. same nat may have different representations *)
(* e.g. : Z, A Z, A Z Z *)
...
(* Theorem norm_bin_nat_bin: forall b : bin, 
  normalize b = nat_to_bin (bin_to_nat b).
Proof. Admit Obligations. *)
