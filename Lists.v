...
Fixpoint count (v:nat) (s:bag) : nat :=
  match s with 
  | nil => 0 
  | x :: xs => count v xs + match x =? v with 
                            | false => 0
                            | true => 1
                            end
  end.
...
Proof.
  ...
  - simpl. rewrite IHl1. 
    assert (H: forall a b c : natlist, (a ++ b) ++ c = a ++ b ++ c). {
      intros. induction a.
      - reflexivity.
...
Proof.
  ...
  intros. repeat rewrite H. reflexivity.
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | [], _ => false 
  | _, [] => false 
  | x :: xs, y :: ys => andb (x =? y) (eqblist xs ys)
  end.
...
Proof. intros. induction l.
  - reflexivity.
  - simpl. rewrite IHl. 
    assert (H: forall x : nat, x =? x = true). { 
      intros. induction x. 
      - reflexivity. 
...
Proof.
  intros. induction s.
  - reflexivity.
  - simpl. destruct (count 1 (1 :: s)).
    + discriminate.
    + destruct (n =? 1).
      * induction (count 1 s); reflexivity.
      * induction (count 1 s); reflexivity.
Qed.
(** [] *)
...
Proof.
  assert (H: 0 =? 0 = true). { simpl. reflexivity. }
  assert (G: forall a, S a =? 0 = false). { intros. simpl. reflexivity. }
  assert (Z: forall a, a <=? a + 1 = true). { 
    induction a. 
    - simpl. reflexivity.
    - simpl. rewrite IHa. reflexivity.
  }
  ...
  - simpl. reflexivity.
  - simpl. destruct n.
    + rewrite H. rewrite Z. reflexivity.
    + rewrite G. simpl. repeat rewrite <- plus_n_O. assumption.
Qed.
(** [] *)
...
Theorem bag_count_sum: forall b1 b2 : bag, forall v : nat, 
  count v b1 + count v b2 = count v (sum b1 b2).
Proof.
  assert (H: forall a b c : nat, a + b + c = a + c + b). {
    intros. induction a.
  ...
  intros. induction b1.
  - reflexivity.
  - simpl sum. simpl. destruct (n =? v).
    + rewrite <- IHb1. rewrite H. reflexivity.
    + rewrite <- IHb1. rewrite H. reflexivity.
Qed.
...
Theorem rev_injective: forall xs ys : natlist, 
  rev xs = rev ys -> xs = ys.
Proof.
...
Proof. intros. destruct x. simpl. induction n; auto. 
Qed. 
(** [] *)
...
Proof.
  intros. simpl. destruct (eqb_id x y).
  - discriminate.
  - reflexivity.  
Qed.
(** [] *)
...
(* suppose we have n baz, then we have the recursive function
f n = n + n * 2
the only fix point is 0 *)

(* Do not modify the following line: *)
Definition manual_grade_for_baz_num_elts : option (nat*string) := None.
(** [] *)

(* Tue Oct 8 15:58:01 EDT 2019 *)
