...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  assert (H': forall xs ys : list nat, xs = ys -> rev xs = rev ys). {
    intros. destruct xs.
    - rewrite H. reflexivity.
    - rewrite H. reflexivity.
  } 
  intros. apply H' in H. rewrite rev_involutive in H. symmetry. apply H. 
Qed.
(** [] *)
...
Proof.
  ...
  rewrite H1 in H. 
  injection H. intros. 
  symmetry in H3. transitivity z; assumption.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct m.
    + discriminate.
    + simpl in H. apply IHn in H. apply f_equal. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl in H. destruct m.
    + reflexivity. 
    + discriminate.
  - intros. rewrite <- plus_n_Sm in H. simpl in H. destruct m.
    + discriminate.
...
Proof.
  ...
  - destruct n.
    + intros. discriminate.
    + intros. simpl in H. injection H.
      * intros. apply IHl in H0. simpl. assumption.
Qed. 
...
Definition split_combine_statement : Prop := forall X Y, forall (xs : list X) (ys : list Y), 
 length xs = length ys -> split (combine xs ys) = (xs, ys).

Theorem split_combine : split_combine_statement.
Proof.
  intros X Y xs. induction xs as [ | x xs' IHxs]; intros ys H.
    - destruct ys; simpl in *.
      + reflexivity.
      + discriminate.
    - destruct ys; simpl in *. 
      + discriminate.
      + inversion H. apply IHxs in H1. rewrite H1. reflexivity.
Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     test x = true.
Proof.
  intros. generalize dependent x. induction l as [ | h t IHl].
    - intros. discriminate H.
    - simpl. destruct (test h) eqn:E.
      + intros. inversion H. rewrite H1 in E. assumption.
      + intros. apply IHl in H. assumption.
Qed.
(** [] *)
...
Proof. 
  intros. induction l.
    - reflexivity.
    - simpl. destruct (test x) eqn: HeqTx.
      + unfold existsb'. simpl. rewrite HeqTx. simpl. reflexivity.
      + rewrite IHl. unfold existsb'. simpl. rewrite HeqTx. simpl. reflexivity.
Qed.
